...
export const calculateProjectProgress = (project) => {
  ...
  totalRequired++;
  if (isOtherFieldValid(formData[FIELDS.ENTITY_TYPE], formData[FIELDS.ENTITY_TYPE_OTHER]))
    completed++;
  totalRequired++;
  ...
  totalRequired++;
  if (isOtherArrayFieldValid(formData[FIELDS.INDUSTRIES], formData[FIELDS.INDUSTRY_OTHER]))
    completed++;
  totalRequired++;
  ...
  if (
    isOtherArrayFieldValid(formData[FIELDS.MAJOR_DECISIONS], formData[FIELDS.MAJOR_DECISIONS_OTHER])
  )
    completed++;
  totalRequired++;
  ...
  totalRequired++;
  if (isOtherFieldValid(formData[FIELDS.VESTING_SCHEDULE], formData[FIELDS.VESTING_SCHEDULE_OTHER]))
    completed++;
  totalRequired++;
  ...
  const isSectionDone = (sectionId) => {
    switch (sectionId) {
      ...
      case SECTION_IDS.EQUITY_ALLOCATION: {
        ...
        if (
          !entries.every(
            (e) => e[FIELDS.EQUITY_ENTRY_NAME] && e[FIELDS.EQUITY_ENTRY_PERCENTAGE] !== '',
          )
        )
          return false;
        const total = entries.reduce(
          (s, e) => s + (parseFloat(e[FIELDS.EQUITY_ENTRY_PERCENTAGE]) || 0),
          0,
        );
        if (Math.abs(total - 100) > 0.01) return false;
        return (
          collaboratorIds.length > 0 &&
          collaboratorIds.every((id) => formData[FIELDS.ACKNOWLEDGE_EQUITY_ALLOCATION]?.[id])
        );
      }
      ...
      case SECTION_IDS.DECISION_MAKING: {
        const shotgunOk =
          formData[FIELDS.INCLUDE_SHOTGUN_CLAUSE] === 'Yes'
            ? collaboratorIds.length > 0 &&
              collaboratorIds.every((id) => formData[FIELDS.ACKNOWLEDGE_SHOTGUN_CLAUSE]?.[id])
            : true;
        return !!(
          isOtherArrayFieldValid(
      ...
      case SECTION_IDS.IP:
        return !!(
          formData[FIELDS.HAS_PRE_EXISTING_IP] &&
          collaboratorIds.length > 0 &&
          collaboratorIds.every((id) => formData[FIELDS.ACKNOWLEDGE_IP_OWNERSHIP]?.[id])
        );
      ...
      case SECTION_IDS.NON_COMPETITION:
        return !!(
          collaboratorIds.length > 0 &&
          collaboratorIds.every((id) => formData[FIELDS.ACKNOWLEDGE_CONFIDENTIALITY]?.[id]) &&
          isOtherFieldValid(
            formData[FIELDS.NON_COMPETE_DURATION],
            formData[FIELDS.NON_COMPETE_DURATION_OTHER],
          ) &&
      ...
      case SECTION_IDS.GENERAL_PROVISIONS:
        return !!(
          ...
          collaboratorIds.every(
            (id) => formData[FIELDS.ACKNOWLEDGE_AMENDMENT_REVIEW_REQUEST]?.[id],
          ) &&
