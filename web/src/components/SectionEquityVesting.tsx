...
function SectionEquityVesting({
  ...
  return (
    <div>
      ...
      >
        When Robin and Antje started Zipcar, they decided to split ownership 50/50. Fair and simple.
        At first, it seemed like a success story: the company grew, became highly profitable, and
      ...
      >
        But within a year and a half, conflicts caused Robin to fire Antje. Except, Antje kept her
        50% stake in the company. Robin was stuck working around the clock for virtually no pay.
      </p>
      <p
        style={{
          fontSize: '14px',
      ...
      >
        This is exactly why vesting exists. Without it, a cofounder can walk away with a huge slice
        of the company without putting in the work, leaving the remaining cofounders carrying the
        burden. Vesting means you earn your equity over time by building the company.
      </p>
      ...
      <div>
        <QuestionCard
          ...
          tooltip={QUESTION_CONFIG[FIELDS.VESTING_START_DATE].tooltip}
        ...
        <QuestionCard
          ...
          answerPreview={getPreview(
            FIELDS.VESTING_SCHEDULE,
            formData,
            FIELDS.VESTING_SCHEDULE_OTHER,
          )}
          tooltip={QUESTION_CONFIG[FIELDS.VESTING_SCHEDULE].tooltip}
          standard={QUESTION_CONFIG[FIELDS.VESTING_SCHEDULE].standard}
        ...
        <QuestionCard
          question={QUESTION_CONFIG[FIELDS.CLIFF_PERCENTAGE].question}
          answerPreview={
            formData[FIELDS.CLIFF_PERCENTAGE] ? `${formData[FIELDS.CLIFF_PERCENTAGE]}%` : ''
          }
          tooltip={QUESTION_CONFIG[FIELDS.CLIFF_PERCENTAGE].tooltip}
          standard={QUESTION_CONFIG[FIELDS.CLIFF_PERCENTAGE].standard}
        ...
        >
          {showValidation && !formData[FIELDS.CLIFF_PERCENTAGE] && (
        ...
        <QuestionCard
          question={QUESTION_CONFIG[FIELDS.ACCELERATION_TRIGGER].question}
          answerPreview={getPreview(FIELDS.ACCELERATION_TRIGGER, formData)}
          tooltip={QUESTION_CONFIG[FIELDS.ACCELERATION_TRIGGER].tooltip}
          subQuestion={
        ...
        >
          {showValidation && !formData[FIELDS.ACCELERATION_TRIGGER] && (
            <span className="text-red-700 text-xs">* Required</span>
          )}
          ...
          {formData[FIELDS.ACCELERATION_TRIGGER] === 'Yes' && (
            <div className="conditional-section">
              <QuestionCard
                question="For how long after the acquisition should this protection apply?"
                alwaysExpanded
                flat
              >
        ...
        <QuestionCard
          ...
          answerPreview={
            formData[FIELDS.SHARES_SELL_NOTICE_DAYS]
              ? `${formData[FIELDS.SHARES_SELL_NOTICE_DAYS]} days`
              : ''
          }
        ...
        >
          {showValidation && !formData[FIELDS.SHARES_SELL_NOTICE_DAYS] && (
            <span className="text-red-700 text-xs">* Required</span>
          )}
          <input
            type="number"
        ...
        <QuestionCard
          ...
          answerPreview={
            formData[FIELDS.SHARES_BUYBACK_DAYS]
              ? `${formData[FIELDS.SHARES_BUYBACK_DAYS]} days`
              : ''
          }
        ...
        >
          {showValidation && !formData[FIELDS.SHARES_BUYBACK_DAYS] && (
            <span className="text-red-700 text-xs">* Required</span>
          )}
          <input
            type="number"
        ...
        >
          <p style={{ fontSize: '12px', fontWeight: 300, color: '#666', lineHeight: 1.65 }}>
            <strong style={{ fontWeight: 500 }}>Note:</strong> If you receive unvested shares, you
            can file an 83(b) election within 30 days of incorporation. This lets you pay taxes on
            the current (usually low) value of your shares instead of their future value as they
            vest. Consult a tax advisor.
          </p>
        </div>
      </div>
    </div>
