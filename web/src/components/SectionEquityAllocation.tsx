...
function SectionEquityAllocation({
  ...
  const otherSubmissions = {
    statusList: otherCollaboratorIds.map((id) => ({
  ...
  const handleAddEquityEntry = () => {
    handleChange(FIELDS.EQUITY_ENTRIES, [
      ...equityEntries,
      { [FIELDS.EQUITY_ENTRY_NAME]: '', [FIELDS.EQUITY_ENTRY_PERCENTAGE]: '' },
    ]);
    handleChange(FIELDS.ACKNOWLEDGE_EQUITY_ALLOCATION, {});
  };
  ...
  return (
    <div>
      ...
      >
        A few years ago, two friends from Stanford launched a startup that took off fast. Within six
        months, they were in YC, had a long waiting list, and investors were calling.
      </p>
      <p
        style={{
          fontSize: '14px',
      ...
      >
        But one cofounder started feeling like they were doing more. "I'm the CEO, I'm fundraising,
        I'm working longer hours." They tried to renegotiate the equity split from 50/50 to 70/30.
        The other cofounder felt blindsided, trust collapsed, and by Demo Day they'd split, both
      ...
      >
        Be very reluctant to change equity allocation once you've agreed. The #1 reason for
        cofounder breakups in the most recent YC batch was cofounders trying to revisit a settled
        split.
      </p>
        ...
        <div className="card-bottom">
          <div className="card-bottom-inner">
            <div className="card-hint">
              An optional 3-step tool to help you find your split before you commit to one.
            </div>
            <div style={{ marginTop: 18, display: 'flex', alignItems: 'center', gap: 16 }}>
              <button
                type="button"
                className="eq-open-btn"
                disabled={isReadOnly}
            ...
            <div className="eq-legal-wrap">
              <div className="eq-legal-inner">
                This equity calculator is for informational and planning purposes only. Using it
                does not grant, issue, vest, or transfer any equity, securities, or ownership
                interest of any kind. No equity exists unless and until it is formally approved and
                issued through proper corporate action (e.g., board approval) and documented via
                legally binding agreements (such as a stock purchase agreement, option grant, or
                equity incentive plan). You must complete the required legal and administrative
                steps for any equity to be valid.
              </div>
            </div>
          </div>
        </div>
            ...
            <div style={{ marginTop: 20 }}>
              ...
              {equityEntries.length === 0 ? (
                <p className="text-gray-500 text-sm mb-6">
                  Click "+ Add Cofounder" to add equity allocations
                </p>
              ) : (
                <div id="eq-entry-list">
                  {equityEntries.map((entry, index) => (
                    <div
                      key={index}
                      className="eq-entry-row"
                      style={{
                        overflow: 'visible',
                        position: 'relative',
                        zIndex: equityEntries.length - index,
                      }}
                    >
                      <CustomSelect
                        ...
                        onChange={(value) =>
                          handleEquityEntryChange(index, FIELDS.EQUITY_ENTRY_NAME, value)
                        }
                          ...
                          .filter(
                            (cf) =>
                              !equityEntries.some(
                                (e, i) =>
                                  i !== index &&
                                  e[FIELDS.EQUITY_ENTRY_NAME] === cf[FIELDS.COFOUNDER_FULL_NAME],
                              ),
                          )
                          .map((cf) => ({
                            value: cf[FIELDS.COFOUNDER_FULL_NAME],
                            label: cf[FIELDS.COFOUNDER_FULL_NAME],
                          }))}
                      ...
                      <div className="eq-pct-wrap">
                        <button
                          ...
                          onClick={() => {
                            ...
                            handleEquityEntryChange(
                              index,
                              FIELDS.EQUITY_ENTRY_PERCENTAGE,
                              Math.max(0, current - 0.5).toFixed(1),
                            );
                          }}
                        >
                          −
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            handleEquityEntryChange(
                              index,
                              FIELDS.EQUITY_ENTRY_PERCENTAGE,
                              Math.min(100, current + 0.5).toFixed(1),
                            );
                          }}
                        >
                          +
                      ...
                      {showValidation &&
                        (!entry[FIELDS.EQUITY_ENTRY_NAME] ||
                          !entry[FIELDS.EQUITY_ENTRY_PERCENTAGE]) && (
                          <span className="text-red-700 text-xs eq-entry-validation">
                            * Name and equity % required
                          </span>
                        )}
                    </div>
                  ))}
              ...
              {equityEntries.length > 0 && (
                <>
                  <div className={`eq-total-row${isOff ? ' error' : ''}`}>
              ...
              <button
                ...
                disabled={isReadOnly || !canAddMoreEntries}
              >
                + Add Cofounder
              </button>
              {!canAddMoreEntries && (
                <span style={{ fontSize: '11px', color: '#888', marginLeft: '10px' }}>
                  {cofounderEntries.length === 0
                    ? 'Add cofounders in the Cofounder Info section first.'
                    : 'All cofounders added.'}
                </span>
              )}
              ...
              <div style={{ marginTop: 10 }}>
                {collaboratorIds.map((userId) => {
                  const isApproved =
                    formData[FIELDS.ACKNOWLEDGE_EQUITY_ALLOCATION]?.[userId] || false;
                  ...
                  return (
                    ...
                    >
                      <div className="eq-sign-check">
                        <span className="eq-sign-checkmark">✓</span>
                      </div>
                      <div className="eq-sign-label">
                        I acknowledge and accept this equity allocation:{' '}
                        <span className="eq-sign-name">{getCollaboratorName(userId)}</span>
                        {isAdmin(userId) && (
                          <span style={{ marginLeft: 6, fontSize: 11, color: '#aaa' }}>
                            (Admin)
                          </span>
                        )}
                        {showValidation && !isApproved && (
