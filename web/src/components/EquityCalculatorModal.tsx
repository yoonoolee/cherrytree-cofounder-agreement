...
function EquityCalculatorModal({
  ...
  const mySplit = useMemo(
    () => calculateSplit(importance, scores, numCofounders),
  ...
  return createPortal(
    <div
      className="eq-modal-backdrop open"
      onClick={(e) => {
    ...
    >
      <div className="eq-modal">
        ...
        <div className="eq-step-indicator">
          <div
            className={`eq-step-dot${step === 'compare' ? ' done' : ' active'}`}
        ...
        {step === 'quiz' && (
          <div className="eq-panel active">
            ...
            <div className="eq-quiz-group">
              ...
              <div className="eq-slider-wrap">
                <input
                  ...
                  min={0}
                  max={10}
                  value={importance[cat.name] || 0}
                  onChange={(e) => updateImportance(Number(e.target.value))}
                />
            ...
            <div className="eq-step-nav">
              <button className="eq-back-btn" onClick={goBack}>
                {isFirst ? 'Cancel' : '← Back'}
              </button>
              <button className="eq-next-btn" onClick={goNext}>
          ...
          <div className="eq-panel active">
            {showLockState ? (
              <div className="eq-lock-state">
                ...
                <div className="eq-lock-desc">
                  Once all cofounders have scored, you'll see their suggested splits too. Your
                  answers stay private until they submit.
                </div>
                <div className="eq-submit-status">
                  {otherSubmissions.statusList.map((s, i) => (
                    <div key={i} className="eq-status-item">
                      <span
                        className={`eq-status-dot ${s.submitted ? 'submitted' : 'pending'}`}
                      ></span>
                      {s.name}
                      {s.submitted ? ' — submitted ✓' : ' — working…'}
                    </div>
                  ))}
                </div>
                <button className="eq-next-btn" onClick={() => setPreviewAnyway(true)}>
                  Preview comparison →
                </button>
              </div>
            ) : (
                  ...
                  {cofounderNames.map((name, ci) => {
                    ...
                    return (
                      <div key={ci} className="eq-suggested-row">
                        ...
                        <div className="eq-suggested-bar-track">
                          <div className="eq-suggested-bar-fill" style={{ width: `${pct}%` }} />
                        </div>
                  ...
                  return (
                    <div key={c.name} className="eq-compare-category">
                      <div className="eq-compare-cat-name">
                        {c.name}
                        <span className={`eq-compare-delta ${delta >= 4 ? 'big' : 'small'}`}>
                          Δ {delta}
                        </span>
                      </div>
                      {cofounderNames.map((name, ci) => (
                          ...
                          {cofounderNames.map((name, ci) => {
                            ...
                            return (
                              <div key={ci} className="eq-suggested-row">
                                ...
                                <div className="eq-suggested-bar-track">
                                  <div
                                    className="eq-suggested-bar-fill"
                                    style={{ width: `${pct}%` }}
                                  />
                                </div>
                                <span className="eq-suggested-pct">{pct.toFixed(1)}%</span>
            ...
            <div className="eq-step-nav">
              ...
              >
                {mySplit && (
                  <button className="eq-next-btn" onClick={() => onUseSplit(mySplit)}>
                    Use this split →
                  </button>
                )}
                <button
