    ...
    <div className="lp-fp" style={{ backgroundImage: "url('/images/ambient.png?v=2')" }}>
      ...
      <div className="lp-fp-content">
        ...
        <div className="lp-s1-card">
          ...
          <div className="lp-s1-row">
            <span className="lp-dot green" />
            <span className="lp-s1-email">you@email.com</span>
            <span className="lp-badge owner">Owner</span>
          </div>
          <div className="lp-s1-row">
            <span className="lp-dot light" />
            <span className="lp-s1-email">alex@startup.co</span>
            <span className="lp-badge added">Added</span>
          </div>
          {showNew && (
            <div className="lp-s1-row lp-s1-new">
              <span className="lp-dot light" />
              <span className="lp-s1-email">{S1_EMAILS[emailIdx % S1_EMAILS.length]}</span>
              <span className="lp-badge added">Added</span>
            </div>
          )}
          <div className="lp-s1-input-row">
            <div className="lp-s1-input">
              <span>{typed}</span>
              {!showNew && <span className="lp-icursor" />}
            </div>
      ...
      <div className="lp-fp-content">
        ...
        <div className="lp-s2-card">
          ...
          {S2_QUESTIONS.map((q, i) => (
            <div
              key={i}
              className={`lp-s2-q${answered.includes(i) ? ' answered' : ''}${activeQ === i ? ' active' : ''}`}
            >
              ...
              <div className="lp-s2-body">
                <div className="lp-s2-body-inner">
                  <div className="lp-s2-input">
                    <span>{activeQ === i ? typed : ''}</span>
                    <span className="lp-icursor lp-dark" />
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="lp-s3-card">
          ...
          <div className="lp-s3-slider-row">
            <div className="lp-s3-track">
              <div
                className="lp-s3-fill"
                style={{
                  width: `${sliderPct}%`,
                  transition: sliderTransition ? 'width 0.7s cubic-bezier(0.16,1,0.3,1)' : 'none',
                }}
              />
              <div
                className="lp-s3-thumb"
                style={{
                  left: `${sliderPct}%`,
                  transition: sliderTransition ? 'left 0.7s cubic-bezier(0.16,1,0.3,1)' : 'none',
                }}
              />
            </div>
      ...
      <div className="lp-fp-content">
        ...
        <div className="lp-s4-card">
          <div className="lp-s4-expert-row">
            ...
            <div>
              <div className="lp-s4-name">Tim</div>
              <div className="lp-s4-role">Cofounder Coach</div>
            </div>
          </div>
          <div className="lp-s4-msgs">
            {S4_MSGS.map(
              (m, i) =>
                shown.includes(i) && (
                  <div key={i} className={`lp-s4-msg${m.sent ? ' user' : ''}`}>
                    {m.text}
                  </div>
                ),
            )}
            {typingIdx !== null && (
              <div className="lp-s4-typing">
                <div className="lp-s4-dot-anim" />
                <div className="lp-s4-dot-anim" />
                <div className="lp-s4-dot-anim" />
              </div>
            )}
...
function HeroVisual() {
  ...
  useEffect(() => {
    ...
    const runScene0 = () => {
      setEqText('');
      setEqCursorOff(false);
      const answer = 'We agreed on an equal 50 / 50 split.';
      let i = 0;
      const step = () => {
        if (i < answer.length) {
          i++;
          setEqText(answer.slice(0, i));
          t(step, 44);
        } else
          t(() => {
            setEqCursorOff(true);
            t(() => clickContinue('c1', () => goTo(1)), 700);
          }, 900);
      };
      t(step, 500);
    };
    ...
    <div className="lp-hero-visual lp-rv">
      ...
      <div className="lp-hv-body">
        <div className="lp-hv-sidebar">
          <div className="lp-hv-logo">
            <div className="lp-hv-logo-mark" />
            <div className="lp-hv-logo-name">Cherrytree</div>
          </div>
          <div className="lp-hv-prog-lbl">{sidebar.lbl}</div>
          <div className="lp-hv-prog-bar">
            <div className="lp-hv-prog-fill" style={{ width: sidebar.prog }} />
          </div>
          {HV_NAV_ITEMS.map((label, i) => (
            <div key={i} className={`lp-hv-nav-item${i === sidebar.navActive ? ' active' : ''}`}>
              <span
                className={`lp-hv-dot${i < sidebar.navActive ? ' done' : i === sidebar.navActive ? ' act' : ''}`}
              />
              {label}
            </div>
          ))}
        ...
        <div className="lp-hv-main">
          <div className={screenCls(0)}>
            ...
            <div className="lp-hv-q-hint">
              Describe your approach or run the equity calculator to surface each founder's
              contribution.
            </div>
            <div className="lp-hv-input-field focused">
              <span>{eqText}</span>
              <span className={`lp-hv-inp-cursor${eqCursorOff ? ' off' : ''}`} />
            </div>
            <button className={`lp-hv-continue${clicking === 'c1' ? ' clicking' : ''}`}>
              Continue →
            </button>
          </div>
          <div className={screenCls(1)}>
            ...
            <div className="lp-hv-q-hint">
              Most startups use a 4-year vesting schedule with a 1-year cliff to protect both
              founders.
            </div>
            <div className="lp-hv-options">
              {['2 years', '4 years', '6 years'].map((label, i) => (
                <div key={i} className={`lp-hv-opt${vestSel === i ? ' selected' : ''}`}>
                  {label}
                </div>
              ))}
            </div>
            <button className={`lp-hv-continue${clicking === 'c2' ? ' clicking' : ''}`}>
              Continue →
            </button>
          </div>
          <div className={screenCls(2)}>
            ...
            <div className="lp-hv-q-hint">
              Agreeing on a framework now prevents costly disputes later.
            </div>
            <div className="lp-hv-options">
              {['Unanimous vote', 'Majority vote', 'CEO decides'].map((label, i) => (
                <div key={i} className={`lp-hv-opt${decSel === i ? ' selected' : ''}`}>
                  {label}
                </div>
              ))}
            </div>
            <button className={`lp-hv-continue${clicking === 'c3' ? ' clicking' : ''}`}>
              Continue →
            </button>
          </div>
          <div className={screenCls(3)}>
            ...
            <div className="lp-hv-q-hint">
              All IP created for the company should be assigned to the entity, not individuals.
            </div>
            ...
            <button className={`lp-hv-continue${clicking === 'c4' ? ' clicking' : ''}`}>
              Continue →
            </button>
          </div>
        </div>
...
const FEATURES = [
  ...
  },
  {
    ...
    desc: 'Use our proprietary equity calculator to determine ownership. Instant, precise splits so everyone knows their stake.',
  },
  {
    num: '04',
    panel: 3,
  ...
  },
  {
    ...
    desc: "We are a team of cofounder coaches, founders, and attorneys ready to help. You're in good hands every step of the way.",
  },
];
...
const CAROUSEL_SLIDES = [
  {
    ...
    tag: 'Universities',
    title: 'Help student teams start right.',
    desc: 'Give entrepreneurship programs a structured tool for the conversations students skip.',
  },
  {
    img: '/images/individuals.png',
    tag: 'Individuals',
    title: "Know what you're getting into.",
    desc: 'Think through every dimension of the partnership before you commit to a cofounder.',
  },
  {
    img: '/images/accelerators.png',
    tag: 'Accelerators',
    title: 'Give your cohort a foundation.',
    desc: 'Surface cofounder misalignment in your portfolio before Demo Day pressure takes over.',
  },
  {
    img: '/images/first-time-founders.png',
    tag: 'First-time founders',
    title: "You don't know what you don't know.",
    desc: 'Walk through every section with the context you need — no legal background required.',
  },
  {
    img: '/images/serial-founders.png',
    tag: 'Serial founders',
    title: "You've been burned before.",
    desc: 'A fast, structured way to get alignment on paper — without going back to lawyers for the basics.',
  },
];
    ...
    <div className="lp">
      ...
      <section className="lp-hero">
        ...
        <h1 className="lp-hero-headline">
          <span className="lp-hl-line">
            <span className="lp-hl-inner">Great companies start</span>
          </span>
          <span className="lp-hl-line lp-d1">
            <span className="lp-hl-inner">
              <em>
                {typedHero}
        ...
        <p className="lp-hero-sub">
          Answer guided questions with your cofounders and get a complete Cofounder Agreement. No
      ...
      <section className="lp-features" id="features">
        <div className="lp-feat-header">
          <h2 className="lp-rv lp-d1">
            Built for <br />
            <em>early-stage</em> cofounders.
          </h2>
          <p className="lp-rv lp-d2">
      ...
      <section className="lp-stats">
        <div className="lp-stats-inner">
          <div className="lp-stats-left">
            ...
            <p className="lp-rv lp-d2">
              Founding teams across the country have used Cherrytree to get aligned before they
              build. No lawyers required. No legal background needed.
            </p>
          </div>
          ...
          {[
            {
              ...
              quote:
                '"We thought we were aligned on equity until Cherrytree showed us we weren\'t. It surfaced a real disagreement before it became a real problem. Worth every minute."',
            },
            {
              init: 'J',
              name: 'James T.',
              role: 'Founder, YC S24',
              quote:
                '"Our accelerator recommended this and I\'m glad they did. We got through IP and vesting in one sitting and actually understood what we were agreeing to."',
            },
            {
              init: 'S',
              name: 'Sarah K.',
              ...
              quote:
                '"The equity calculator alone is worth it. We scored each other independently and compared — it was the most honest conversation we\'d had about the company."',
            },
          ].map((t, i) => (
            <div
              key={i}
              className={`lp-testi-card lp-rv lp-d${i}`}
              style={{ backgroundImage: "url('/images/testimonial-paper.jpg')" }}
            >
      ...
      <section className="lp-carousel" id="who">
        <div className="lp-carousel-header lp-rv">
          <div>
            <h2 className="lp-carousel-title">
              Built for every kind of <em>founding team.</em>
            </h2>
          </div>
        ...
        <div className="lp-carousel-track-wrap">
          <div
            ...
            style={{ transform: `translateX(calc(-${carouselIdx} * 380px))` }}
          >
            {CAROUSEL_SLIDES.map((s, i) => (
              <div
                key={i}
                className={`lp-cs-slide${i === carouselIdx ? ' active' : ''}`}
        ...
        <div className="lp-carousel-dots">
          {CAROUSEL_SLIDES.map((_, i) => (
            <button
              key={i}
              className={`lp-cs-dot${i === carouselIdx ? ' active' : ''}`}
        ...
        <div className="lp-pricing-grid">
          {PRICING.map((p, i) => (
            ...
            >
              ...
              <div className={`lp-pricing-price${p.enterprise ? ' lp-pricing-price-custom' : ''}`}>
                {p.price}
              </div>
