...
- Phase: 5 (Web → TS, leaf-first) — **5a done** (25 commits 43effe9…58368b6, ⏸ checkpoint 2026-09-19); 5b not started
- Step: 5b (leaf/mid components) — first action: `calculateSplit` test beside `EquityCalculatorModal`, then convert leaf components one per commit; `ProWaitlistForm` gets the `proWaitlist` typed collection + shared `ProWaitlistSignup` type; `PaymentModal` drops `sessionStorage.paymentStartTime` (fix)
- Last green commit: 58368b6; `npm run check` 0 errors / 21 warnings / 227 tests (33 files); `npm run test:rules` 14 tests; knip clean except user-event, `calculateSplit`, and `QuestionOption`/`EquityCell` (consumers convert in 5b/5e)
- Web still in JS: 50 files (all components + pages + `App.jsx`/`main.jsx`); every non-component module is TS
...
- Next action: user "continue" → 5b (same session is fine; context still moderate). `origin/master` still at d12eb6c as of 2026-09-19 08:20 PDT
- Blocked on: nothing. **Not yet deployed to dev**: the 5a callable change (limited-use App Check tokens + server replay rejection) must go out as functions + hosting together (`npm run deploy:dev`) — Phase 5e smoke
...
- [ ] **Stripe `cherrytree-prod-dev` endpoint** (test mode, account "Cherrytree Technologies, LLC" `acct_1Si69sEbFwh64Boe`) points at **prod's** `stripeWebhook` and prod accepts it (HTTP 200 on 2026-09-19 07:20:58Z) → prod's `STRIPE_WEBHOOK_SECRET` is that *test* endpoint's signing secret. Decide with the teammate: disable that endpoint (recommended) and confirm which **live-mode** endpoint/secret prod should verify. Until then, any test purchase with a plain project name creates a real project + Clerk org in prod (this one only failed because old prod code double-escaped `&`).
...
- **Prod deploy (Phase 4):** the deploy must delete the retired `deleteAccount` function (`firebase functions:delete deleteAccount --region us-west2` or `firebase deploy --force`); and settle the Stripe `cherrytree-prod-dev` test-mode endpoint that targets prod (Status → "Open from Phase 4") before enabling the new webhook code on prod.
...
- [ ] `lastOpened` → `serverTimestamp()` (today a JS `Date` client-side, `src/pages/SurveyPage.jsx`) — 5e with `SurveyPage`; ~~`updatedAt` written on every project save~~ → **removed** instead (5a, bc48198; decision above)
...
- [ ] Phase 5e full smoke must include the three checks skipped in Phase 4 (Status → "Open from Phase 4") **plus**: every callable works with limited-use tokens on the dev server (debug token) and on dev hosting (reCAPTCHA v3); a second click after a failed call is not rejected as a replay
