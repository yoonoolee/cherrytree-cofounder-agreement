...
- Phase: 3 (TypeScript foundation + shared package) — not started
- Step: —
- Last green commit: 3188622 (Phase 2 tagged refactor/p2-done at c33cf81)
- Next action: Phase 3 step 1 — tsconfig.base.json + web/functions/shared tsconfigs + root `typecheck` script
...
- [ ] **Phase 3 — TypeScript foundation + `shared` package** — 1. `tsconfig.base.json` (strict, `noUncheckedIndexedAccess`, `verbatimModuleSyntax`, `isolatedModules`, `moduleResolution: bundler`, ES2022, `skipLibCheck`; `exactOptionalPropertyTypes` off) + `web/tsconfig.json` (`allowJs: true, checkJs: false`, `@/*` alias mirrored in `vite.config.ts`, `vite-env.d.ts` with typed `ImportMetaEnv`) + `functions/tsconfig.json` (`paths` → `../shared/src`) + `shared/tsconfig.json`; root `typecheck` script in `check`. 2. `shared/` (`@cherrytree/shared`, source-exported, zero deps): `domain/` types (`Project`, `Collaborator`, `UserDoc`, `SurveyData` incl. `Compensation {who, amount}`, ack maps — mutable), `time.ts` (`TimestampLike`, `DateLike`, `toDate`), `survey/` (`FIELDS`, `COLLABORATOR_FIELDS`, options, `SURVEY_FIELDS`, `INITIAL_FORM_DATA`, section ids/order/metadata, ack lists from the server, `OTHER_FIELD_CONFIG` + nested `mergeOtherFields` verbatim from the server), `callables.ts` (`CallableMap`). Tests in TS: `INITIAL_FORM_DATA` key set/defaults, `mergeOtherFields` flat/nested/mutation-pinned, `toDate`. One-off move verification in the commit message (shared `INITIAL_FORM_DATA` == old client; `OTHER_FIELD_CONFIG`/ack lists == old server). 3. Point web (workspace) and functions (esbuild alias) at shared; delete old `surveySchema` copies + `web/src/config/sectionConfig.js`; `functions/esbuild.config.mjs` (CJS bundle, externals from `functions/package.json`, alias), `main: lib/index.js`, `firebase.json` predeploy; temporary `functions/src/index.ts` = `export * from '../index.js'` so the bundle path is proven now. 4. Root vitest projects `web`, `shared`, `functions`. Gate + emulator lists 10 + golden matches. ⏸
