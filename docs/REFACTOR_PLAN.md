...
- Phase: 5 (Web → TS, leaf-first) — **not started**; Phase 4 done and tagged `refactor/p4-done`
- Step: 5a (non-component modules) — first action is the tests for `lib/functions.ts` / utils / hooks, then rename + annotate
- Last green commit: see `git log -1` (tracker/docs commits after 07ba08c rules); `npm run check` 0 errors / 36 warnings / 130 tests; `npm run test:rules` 13 tests; emulator lists 9
...
- Next action: fresh session → `git fetch origin` (merge `origin/master` if it moved past d12eb6c) → gate → start 5a
- Blocked on: nothing
...
- [ ] Phase 5e full smoke must include the three checks skipped in Phase 4 (Status → "Open from Phase 4")
